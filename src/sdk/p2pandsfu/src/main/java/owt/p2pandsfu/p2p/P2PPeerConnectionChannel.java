...
import static owt.p2pandsfu.p2p.OwtP2PError.P2P_CLIENT_INVALID_STATE;
import static owt.p2pandsfu.p2p.OwtP2PError.P2P_WEBRTC_SDP;
...
final class P2PPeerConnectionChannel extends PeerConnectionChannel {
    ...
    // <MediaStreamId, P2PRemoteStream>
    private ConcurrentHashMap<String, P2PRemoteStream> remoteStreams;
    // <MediaStreamId>
    private ArrayList<String> pendingAckRemoteStreams;
    private ArrayList<P2PPublication> publications;
    ...
    void publish(LocalStream localStream, ActionCallback<P2PPublication> callback) {
    ...
    protected synchronized void dispose() {
        super.dispose();
        for (P2PRemoteStream remoteStream : remoteStreams.values()) {
            remoteStream.onEnded();
        }
        for (P2PPublication publication : publications) {
            publication.onEnded();
        }
    ...
    void processTrackAck(JSONArray tracksData) throws JSONException {
        for (int i = 0; i < tracksData.length(); i++) {
            ...
            if (callbackInfo != null
                ...
                P2PPublication publication = new P2PPublication(callbackInfo.mediaStreamId, this);
    ...
    public void onAddStream(final MediaStream mediaStream) {
        callbackExecutor.execute(() -> {
            P2PRemoteStream remoteStream = new P2PRemoteStream(key, mediaStream);
    ...
    static class CallbackInfo {
        ...
        CallbackInfo(MediaStream mediaStream, ActionCallback<P2PPublication> callback) {
